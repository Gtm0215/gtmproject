...
def create_user(username, password, name, age, gender, height_cm, weight_kg, activity_level, medical_conditions):
    conn = get_conn()
    c = conn.cursor()
    c.execute("""
        INSERT INTO users (username, password_hash, name, age, gender, height_cm, weight_kg, activity_level, medical_conditions)
        VALUES (?,?,?,?,?,?,?,?,?)
    """, (username, hash_password(password), name, age, gender, height_cm, weight_kg, activity_level, medical_conditions))
    conn.commit()
    uid = c.lastrowid
    conn.close()
    return uid
...
def recommend_exercises(bmi, medical_conditions):
    ...
    if bmi is None:
        return ["Missing data"]
    if bmi < 18.5:
        recs.append("Strength training 3x/week — include protein-rich meals.")
    elif bmi < 25:
        recs.append("Balanced routine: cardio 3x/week + strength 2x/week.")
    elif bmi < 30:
        recs.append("Moderate cardio (30-45 min daily) + light strength 2x/week.")
    else:
        recs.append("Low-impact cardio (walking, swimming); consult doctor first.")
    if "diab" in conds:
        recs.append("Regular brisk walking and resistance training help blood sugar control.")
    if "heart" in conds or "hypert" in conds:
        recs.append("Keep activity low-to-moderate; consult cardiologist.")
    if "back" in conds:
        recs.append("Focus on core and mobility, avoid heavy spinal loads.")
    return recs
...
def recommend_diet(bmi, medical_conditions, calorie_target):
    ...
    if bmi < 18.5:
        ...
        eat += ["Low-calorie high-volume foods", "Legumes, lean protein"]
    ...
    if "hypert" in conds or "blood pressure" in conds:
        eat += ["Potassium-rich foods, whole grains"]
        avoid += ["High-sodium processed foods"]
    if calorie_target:
        eat.append(f"Target calories ≈ {calorie_target} kcal/day")
    return {"eat": list(dict.fromkeys(eat)), "avoid": list(dict.fromkeys(avoid))}
...
st.caption("Sign up, login, track health, and get personalized diet & exercise suggestions.")
        ...
        with st.form("profile_edit"):
            ...
            if save:
                update_profile(st.session_state.user_id, {
                    ...
                    "weight_kg": weight, "activity_level": activity, "medical_conditions": medical
                })
